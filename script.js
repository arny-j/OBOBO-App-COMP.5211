...
function updateDisplay(elementId) {
  document.querySelector(`#${elementId} .time-display`).textContent =
    formatTime(time);
}
function startStop(timerId) {
  ...
  if (isRunning) {
    ...
    interval = setInterval(() => {
      time += 10; // Increased from 1 to 10 to increment correctly
...
function reset(timerId) {
  clearInterval(interval);
  time = 0; // Changed to 0 to correctly reset time values
  isRunning = false;
  updateDisplay(timerId);
...
document
  ...
  .addEventListener("click", () => startStop("stopwatch"));
document
  .getElementById("stopwatch-reset")
  .addEventListener("click", () => reset("stopwatch"));
// Lap Timer
document
  .getElementById("laptimer-startstop")
  .addEventListener("click", () => startStop("laptimer"));
document
  .getElementById("laptimer-reset")
  .addEventListener("click", () => reset("laptimer"));
